...
var map_refresh_promise = null;
var map_refresh_pending = false;
...
async function fetch_map_and_refresh() {
	if (map_refresh_promise != null) {
		map_refresh_pending = true;
		return map_refresh_promise;
	}

	map_refresh_promise = (async function() {
		try {
			do {
				map_refresh_pending = false;
			...
			} while (map_refresh_pending);
		} finally {
			map_refresh_promise = null;
		}
	})();
	return map_refresh_promise;
}
...
async function move_request(dy, dx) {
	try {
		...
		const result = await response.json();
		return { status: "ok", y: result.y, x: result.x };
	} catch (error) {
		console.error('Error:', error);
		return { status: "error" };
	}
...
var gameArea = {
	...
	bind_key_events: function () {
		...
		var key_handler = async function (e) {
			await me.handle_key(e);
		};
	...
	bind_mouse_events: function () {
		var me = this;
		var mouse_handler = async function (e) {
			await me.handle_mouse(e);
		};
	...
	move_character: async function(dy, dx) {
		const oldY = posY;
		const oldX = posX;
		...
		if (! last_blocking(kinds)) {
			...
			await fetch_map_and_refresh();
			return;
		}

		const result = await move_request(dy, dx);
		if (result.status == "ok") {
			posY = result.y;
			posX = result.x;
		} else {
	...
	take_item: async function() {
		await take_request();
		await fetch_map_and_refresh();
	},
	place_item: async function() {
		await place_request();
		await fetch_map_and_refresh();
	},
	handle_key: async function (e) {
		var key = this.get_key(e);
		switch (key) {
			case 65: // A
			case 37: // left arrow
				await this.move_character(-0, -1);
				break;
			case 87: // W
			case 38: // up arrow
				await this.move_character(-1, -0);
				break;
			case 68: // D
			case 39: // right arrow
				await this.move_character(+0, +1);
			...
			case 40: // down arrow
				...
				break;
			case 84: // T
				await this.take_item();
				break;
			case 80: // P
				await this.place_item();
				break;
			case 27: // escape key
				break;
			case 32: // space key
	...
	handle_mouse: async function (e) {
		...
		await use_request(y - playerY, x - playerX);
		await fetch_map_and_refresh();
	},
