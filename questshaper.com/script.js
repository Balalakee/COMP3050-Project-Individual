...
async function fetch_map(y, x) {
	try {	
		console.log("Fetching:", server_addr + `/info?y=${y}&x=${x}&session=test`);
		const response = await fetch(server_addr + `/info?y=${y}&x=${x}&session=test`, {
			method: 'GET',
...
console.log("Sending move request", dy, dx);
	try {
		const response = await fetch(server_addr + `/move?dy=${dy}&dx=${dx}&session=test`, {
			method: 'GET',
